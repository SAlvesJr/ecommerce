...
class Cart extends Model {
	...
	function save(){
		...
		$result = $sql->select("CALL sp_carts_save(:idcart, :dessessionid, :iduser, :deszipcode, :vlfreight, :pnrdays )", [

			":idcart"=>$this->getcart(),
			":dessessionid"=>$this->getdessessionid(),
			":iduser"=>$this->getiduser(),
			":deszipcode"=>$this->getdeszipecode(),
			":vlfreight"=>$this->getvlfreght(),
			":pnrdays"=>$this->getnrdays()
		]);

		$this->setDate($result[0]);
	}
}
